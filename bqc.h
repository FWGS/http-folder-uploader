...
void __attribute__((optimize(0))) *memset(void *sp, int c, size_t n){u8 *s=sp;while (n--)*s++=c;return sp;}
...
static inline size_t __attribute__((optimize(0))) strlen(const char *s){size_t i=0; while(s[i])++i;return i;}
static inline int strncmp(const char *s1, const char *s2, size_t len){while (*s1 && --len && *s1==*s2){++s1;++s2;}return *s2-*s1;}
static __attribute__((optimize(0))) inline char * strncpy(char * d, const char *s, size_t n){char *r=d;if (n) do {*d++=*s;s+=!!*s;} while (--n && *(d-1));return r;}
