...
inline __attribute__((used)) __attribute__((always_inline)) __attribute__((optimize("-Ofast"))) size_t strlen(const char *s){size_t i=0; while(s[i])++i;return i;}
#define strlen __builtin_strlen
static inline const char *getnthstring(const char *s,size_t n){while(n--)while(*s++);return s;}
inline void __attribute__((used))  *memset(void *sp, int c, size_t n){u8 *s=sp;while (n--)*s++=c;return sp;}
__attribute__((used)) inline /*__attribute__((always_inline))*/ void *memcpy(void *dp, const void *sp, size_t n){u8 *d=dp;const u8 *s=sp;while(n--)*d++=*s++;return dp;}
static inline char * strchr(const char *s,int c){do if (*s==c) return (char*)s; while (*s++);return NULL;}
...
inline __attribute__((used)) /*__attribute__((always_inline))*/ char * strncpy(char * d, const char *s, size_t n){char *r=d;if (n) do {*d++=*s;s+=!!*s;} while (--n && *(d-1));return r;}
static inline __attribute__((always_inline)) char *strstr(const char *h, const char *n){size_t len=strlen(n);while(*h){if(*h==*n && !memcmp(h,n,len)) return (char *)h;else h++;}return NULL;}
